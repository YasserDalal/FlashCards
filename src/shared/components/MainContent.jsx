  ...
  return (
    ...
    width: `${questionsAndAnswers.length}00%` }}>
        {questionsAndAnswers.map((item, index) => (
          <div className="w-full flex rounded-lg justify-center items-center bg-slate-600 relative text-[clamp(1rem,5vw,1.5rem)] text-center px-3" key={index}>
            <h2>{item.question}</h2>
            <div className="flex items-center transition-transform duration-300 text-center bg-slate-600 absolute z-10 top-0 bottom-0 left-0 right-0 px-3" style={{ transform: `translateY(${smooth}%)` }}>
